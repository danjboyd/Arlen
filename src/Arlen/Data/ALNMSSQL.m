...
static NSLock *ALNMSSQLODBCLoaderLock(void) {
...
static BOOL ALNMSSQLLoadODBC(NSError **error) {
  NSLock *loadLock = ALNMSSQLODBCLoaderLock();
  [loadLock lock];
  @try {
    if (gODBCHandle != NULL) {
      return YES;
    ...
    if (envCandidate != NULL && envCandidate[0] != '\0') {
      handle = ALNOpenODBCCandidate(envCandidate);
      if (handle == NULL) {
        lastLoadError = [NSString stringWithFormat:@"ARLEN_ODBC_LIBRARY=%s failed: %@",
                                                   envCandidate,
...
#if defined(_WIN32)
          @"libodbc-2.dll, libodbc.dll, odbc32.dll, or ARLEN_ODBC_LIBRARY";
#else
          @"libodbc.so.2, libodbc.so, or ARLEN_ODBC_LIBRARY";
#endif
      gODBCLoadError = [NSString stringWithFormat:
                                     @"ODBC shared library not found (checked %@)%@",
                                     candidateSummary,
