...
static NSString *ALNORMTSRenderRealtimeModule(void) {
  NSMutableString *output = [NSMutableString string];
  [output appendString:@"// Generated by arlen typescript-codegen. Do not edit by hand.\n\n"];
  [output appendString:@"export type ArlenRealtimeTransport = 'websocket' | 'sse' | 'poll';\n"];
  [output appendString:@"export type ArlenRealtimeConnectionState = 'idle' | 'connecting' | 'replaying' | 'live' | 'resync-required' | 'disconnected';\n\n"];
  [output appendString:@"export interface ArlenRealtimeCursor {\n  stream_id: string;\n  sequence: number;\n}\n\n"];
  [output appendString:@"export interface ArlenRealtimeEventEnvelope<TPayload = Record<string, unknown>> {\n  stream_id: string;\n  sequence: number;\n  event_id: string;\n  event_type: string;\n  occurred_at: string;\n  payload: TPayload;\n  idempotency_key?: string;\n  actor?: Record<string, unknown>;\n  metadata?: Record<string, unknown>;\n}\n\n"];
  [output appendString:@"export interface ArlenRealtimeReplayResult<TPayload = Record<string, unknown>> {\n  stream_id: string;\n  events: Array<ArlenRealtimeEventEnvelope<TPayload>>;\n  latest_cursor: ArlenRealtimeCursor;\n  replay_limit: number;\n  replay_window: number;\n  requested_after_sequence?: number;\n  resync_required?: false;\n}\n\n"];
  [output appendString:@"export interface ArlenRealtimeResyncRequired {\n  status: 'resync_required';\n  stream_id: string;\n  latest_cursor: ArlenRealtimeCursor;\n  replay_limit: number;\n  replay_window: number;\n  requested_after_sequence?: number;\n}\n\n"];
  [output appendString:@"export interface ArlenRealtimeClientOptions {\n  baseUrl?: string;\n  fetch?: typeof fetch;\n  streamId: string;\n  cursor?: number | null;\n  replayLimit?: number;\n  replayWindow?: number;\n  transports?: ArlenRealtimeTransport[];\n  pollIntervalMs?: number;\n  websocketFactory?: ((url: string) => WebSocket) | undefined;\n  eventSourceFactory?: ((url: string) => EventSource) | undefined;\n}\n\n"];
  [output appendString:@"export interface ArlenRealtimeConnectionSnapshot {\n  streamId: string;\n  cursor: number | null;\n  desiredTransports: ArlenRealtimeTransport[];\n  activeTransport: ArlenRealtimeTransport | null;\n  state: ArlenRealtimeConnectionState;\n}\n\n"];
  [output appendString:@"function arlenRealtimeDefaultFetch(): typeof fetch {\n  if (typeof globalThis.fetch !== 'function') {\n    throw new Error('ArlenRealtimeStreamClient requires a fetch implementation');\n  }\n  return globalThis.fetch.bind(globalThis);\n}\n\n"];
  [output appendString:@"function arlenRealtimeNormalizeBaseUrl(baseUrl?: string): string {\n  const candidate = (baseUrl ?? '').trim();\n  if (candidate.length === 0) {\n    return '';\n  }\n  return candidate.endsWith('/') ? candidate.slice(0, -1) : candidate;\n}\n\n"];
  [output appendString:@"function arlenRealtimeResolveURL(baseUrl: string, path: string, search?: URLSearchParams): string {\n  const suffix = search && search.toString().length > 0 ? `${path}?${search.toString()}` : path;\n  return baseUrl.length > 0 ? `${baseUrl}${suffix}` : suffix;\n}\n\n"];
  [output appendString:@"function arlenRealtimeNormalizeTransportOrder(transports?: ArlenRealtimeTransport[]): ArlenRealtimeTransport[] {\n  const defaultOrder: ArlenRealtimeTransport[] = ['websocket', 'sse', 'poll'];\n  const input: ArlenRealtimeTransport[] = transports && transports.length > 0 ? transports : defaultOrder;\n  const ordered: ArlenRealtimeTransport[] = [];\n  for (const transport of input) {\n    if (!ordered.includes(transport)) {\n      ordered.push(transport);\n    }\n  }\n  return ordered;\n}\n\n"];
  [output appendString:@"export function arlenRealtimeReplayPath(streamId: string, afterSequence?: number | null, replayLimit = 100, replayWindow = replayLimit): string {\n  const search = new URLSearchParams();\n  if (afterSequence !== undefined && afterSequence !== null) {\n    search.set('after_sequence', String(afterSequence));\n  }\n  search.set('limit', String(replayLimit > 0 ? replayLimit : 100));\n  search.set('replay_window', String(replayWindow > 0 ? replayWindow : (replayLimit > 0 ? replayLimit : 100)));\n  return arlenRealtimeResolveURL('', `/streams/${encodeURIComponent(streamId)}/events`, search);\n}\n\n"];
  [output appendString:@"export function arlenRealtimeTransportSupported(\n  transport: ArlenRealtimeTransport,\n  options: Pick<ArlenRealtimeClientOptions, 'websocketFactory' | 'eventSourceFactory'> = {}\n): boolean {\n  if (transport === 'websocket') {\n    return typeof options.websocketFactory === 'function' || typeof globalThis.WebSocket === 'function';\n  }\n  if (transport === 'sse') {\n    return typeof options.eventSourceFactory === 'function' || typeof globalThis.EventSource === 'function';\n  }\n  return true;\n}\n\n"];
  [output appendString:@"export function arlenRealtimeTransportPlan(options: Pick<ArlenRealtimeClientOptions, 'transports' | 'websocketFactory' | 'eventSourceFactory'> = {}): ArlenRealtimeTransport[] {\n  return arlenRealtimeNormalizeTransportOrder(options.transports).filter((transport) => arlenRealtimeTransportSupported(transport, options));\n}\n\n"];
  [output appendString:@"export function arlenRealtimeNextCursor<TPayload>(events: Array<ArlenRealtimeEventEnvelope<TPayload>>, fallback: number | null = null): number | null {\n  if (!Array.isArray(events) || events.length === 0) {\n    return fallback;\n  }\n  return events[events.length - 1]?.sequence ?? fallback;\n}\n\n"];
  [output appendString:@"export function arlenRealtimeIsResyncRequired(value: unknown): value is ArlenRealtimeResyncRequired {\n  return !!value && typeof value === 'object' && (value as { status?: unknown }).status === 'resync_required';\n}\n\n"];
  [output appendString:@"export class ArlenRealtimeResyncRequiredError extends Error {\n  readonly payload: ArlenRealtimeResyncRequired;\n\n  constructor(payload: ArlenRealtimeResyncRequired) {\n    super(`Arlen realtime resync required for ${payload.stream_id}`);\n    this.name = 'ArlenRealtimeResyncRequiredError';\n    this.payload = payload;\n  }\n}\n\n"];
  [output appendString:@"export class ArlenRealtimeStreamClient<TPayload = Record<string, unknown>> {\n  readonly streamId: string;\n  readonly baseUrl: string;\n  readonly replayLimit: number;\n  readonly replayWindow: number;\n  readonly pollIntervalMs: number;\n  readonly transports: ArlenRealtimeTransport[];\n  private readonly fetchImplementation: typeof fetch;\n  private cursor: number | null;\n  private state: ArlenRealtimeConnectionState;\n  private activeTransport: ArlenRealtimeTransport | null;\n\n  constructor(options: ArlenRealtimeClientOptions) {\n    this.streamId = options.streamId;\n    this.baseUrl = arlenRealtimeNormalizeBaseUrl(options.baseUrl);\n    this.replayLimit = options.replayLimit && options.replayLimit > 0 ? options.replayLimit : 100;\n    this.replayWindow = options.replayWindow && options.replayWindow > 0 ? options.replayWindow : this.replayLimit;\n    this.pollIntervalMs = options.pollIntervalMs && options.pollIntervalMs > 0 ? options.pollIntervalMs : 1000;\n    this.transports = arlenRealtimeNormalizeTransportOrder(options.transports);\n    this.fetchImplementation = options.fetch ?? arlenRealtimeDefaultFetch();\n    this.cursor = options.cursor ?? null;\n    this.state = 'idle';\n    this.activeTransport = null;\n  }\n\n  snapshot(): ArlenRealtimeConnectionSnapshot {\n    return {\n      streamId: this.streamId,\n      cursor: this.cursor,\n      desiredTransports: [...this.transports],\n      activeTransport: this.activeTransport,\n      state: this.state,\n    };\n  }\n\n  resolveTransportPlan(options: Pick<ArlenRealtimeClientOptions, 'websocketFactory' | 'eventSourceFactory'> = {}): ArlenRealtimeTransport[] {\n    return arlenRealtimeTransportPlan({\n      transports: this.transports,\n      websocketFactory: options.websocketFactory,\n      eventSourceFactory: options.eventSourceFactory,\n    });\n  }\n\n  async replay(afterSequence: number | null = this.cursor): Promise<ArlenRealtimeReplayResult<TPayload>> {\n    this.state = 'replaying';\n    const response = await this.fetchImplementation(\n      arlenRealtimeResolveURL(this.baseUrl, arlenRealtimeReplayPath(this.streamId, afterSequence, this.replayLimit, this.replayWindow)),\n      {\n        method: 'GET',\n        headers: {\n          accept: 'application/json',\n        },\n      }\n    );\n    const payload = await response.json() as ArlenRealtimeReplayResult<TPayload> | ArlenRealtimeResyncRequired;\n    if (response.status === 409 && arlenRealtimeIsResyncRequired(payload)) {\n      this.state = 'resync-required';\n      throw new ArlenRealtimeResyncRequiredError(payload);\n    }\n    if (!response.ok) {\n      this.state = 'disconnected';\n      throw new Error(`Arlen realtime replay failed (${response.status})`);\n    }\n    const replayPayload = payload as ArlenRealtimeReplayResult<TPayload>;\n    this.cursor = replayPayload.latest_cursor?.sequence ?? arlenRealtimeNextCursor(replayPayload.events, this.cursor);\n    this.state = 'idle';\n    return replayPayload;\n  }\n\n  async pollOnce(): Promise<ArlenRealtimeReplayResult<TPayload>> {\n    this.activeTransport = 'poll';\n    const replay = await this.replay(this.cursor);\n    this.cursor = replay.latest_cursor?.sequence ?? arlenRealtimeNextCursor(replay.events, this.cursor);\n    this.state = 'live';\n    return replay;\n  }\n\n  async connect(options: Pick<ArlenRealtimeClientOptions, 'websocketFactory' | 'eventSourceFactory'> = {}): Promise<ArlenRealtimeTransport> {\n    this.state = 'connecting';\n    const plan = this.resolveTransportPlan(options);\n    if (plan.length === 0) {\n      this.state = 'disconnected';\n      throw new Error('Arlen realtime client has no supported transports');\n    }\n    const chosen = plan[0] ?? 'poll';\n    this.activeTransport = chosen;\n    if (chosen === 'poll') {\n      await this.pollOnce();\n      return chosen;\n    }\n    this.state = 'replaying';\n    await this.replay(this.cursor);\n    this.state = 'live';\n    return chosen;\n  }\n\n  markCursor(sequence: number | null): void {\n    this.cursor = sequence;\n  }\n}\n"];
  return [NSString stringWithString:output];
}
...
static NSString *ALNORMTSRenderIndexModule(NSArray<NSString *> *targets) {
  ...
  if ([targets containsObject:@"realtime"]) {
    [output appendString:@"export * from './realtime';\n"];
  }
  return [NSString stringWithString:output];
}
...
static NSString *ALNORMTSRenderReadme(NSString *packageName,
  ...
  if ([targets containsObject:@"realtime"]) {
    [output appendString:@"- `src/realtime.ts`: transport-neutral durable stream client contracts and fallback helpers\n"];
  }
  [output appendString:@"\n"];
...
static NSString *ALNORMTSRenderPackageJSON(NSString *packageName,
  ...
  if ([targets containsObject:@"realtime"]) {
    exports[@"./realtime"] = @"./src/realtime.ts";
  }
...
static NSArray<NSString *> *ALNORMTSNormalizedTargets(NSArray<NSString *> *rawTargets,
  ...
  for (NSString *rawTarget in inputs) {
    ...
    for (NSString *rawToken in tokens) {
      ...
      if ([token isEqualToString:@"all"]) {
        for (NSString *expanded in @[ @"models", @"validators", @"query", @"client", @"react", @"meta", @"realtime" ]) {
      ...
      if ([token isEqualToString:@"realtime"]) {
        if (![targets containsObject:@"client"]) {
          [targets addObject:@"client"];
        }
      }
      if (![token isEqualToString:@"models"] && ![token isEqualToString:@"client"] &&
          ...
          ![token isEqualToString:@"meta"] && ![token isEqualToString:@"realtime"]) {
        if (error != NULL) {
          *error = ALNORMMakeError(ALNORMErrorInvalidArgument,
  ...
  if ([targets count] == 0) {
    [targets addObjectsFromArray:@[ @"models", @"validators", @"query", @"client", @"react", @"meta", @"realtime" ]];
  }
...
+ (NSDictionary<NSString *,id> *)renderArtifactsFromModelDescriptors:(NSArray<ALNORMModelDescriptor *> *)descriptors
  ...
  if ([normalizedTargets containsObject:@"realtime"]) {
    files[@"src/realtime.ts"] = ALNORMTSRenderRealtimeModule();
  }
  ...
  for (NSString *path in sortedFilePaths) {
    ...
    if ([path hasSuffix:@"models.ts"]) {
      ...
      kind = @"meta";
    } else if ([path hasSuffix:@"realtime.ts"]) {
      kind = @"realtime";
    }
