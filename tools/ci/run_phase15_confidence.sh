...
output_dir="${ARLEN_PHASE15_OUTPUT_DIR:-$repo_root/build/release_confidence/phase15}"
unit_bundle="${ARLEN_PHASE15_UNIT_BUNDLE:-$repo_root/build/tests/ArlenUnitTests.xctest}"
focused_bundle="${ARLEN_PHASE15_FOCUSED_BUNDLE:-$repo_root/build/tests/Phase15AuthUIOnly.xctest}"
focused_bin="$focused_bundle/ArlenIntegrationTests"
unit_log="$output_dir/phase15_unit.log"
integration_log="$output_dir/phase15_auth_ui_integration.log"
...
clang \
  ...
  "${include_flags[@]}" \
  "$repo_root/tests/integration/Phase13AuthAdminIntegrationTests.m" \
  "${framework_srcs[@]}" \
  "${module_srcs[@]}" \
  "${generated_files[@]}" \
...
if [[ -z "${ARLEN_PG_TEST_DSN:-}" ]]; then
  python3 ./tools/ci/generate_phase15_confidence_artifacts.py \
    --repo-root "$repo_root" \
    --output-dir "$output_dir" \
    --unit-log "$unit_log" \
    --integration-log "$integration_log" \
    --mode skipped \
    --reason "ARLEN_PG_TEST_DSN is unset; focused auth UI bundle compiled and registered, but live Postgres-backed auth mode flows were not exercised"
  echo "ci: phase15 confidence gate skipped live auth UI flow execution because ARLEN_PG_TEST_DSN is unset"
  exit 0
fi
...
echo "ci: phase15 confidence gate complete"
