#!/usr/bin/env python3
"""Generate Phase 11 sanitizer matrix confidence artifacts."""
...
def make_markdown(payload: Dict[str, Any], output_dir: Path) -> str:
    lines: List[str] = []
    lines.append("# Phase 11 Sanitizer Matrix")
    lines.append("")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate Phase 11 sanitizer matrix artifacts")
    parser.add_argument("--repo-root", default=".")
    parser.add_argument("--fixture", default="tests/fixtures/sanitizers/phase11_sanitizer_matrix.json")
    parser.add_argument("--lane-results", required=True)
    parser.add_argument("--output-dir", default="build/release_confidence/phase11/sanitizers")
    ...
    if matrix.get("version") != VERSION:
        raise SystemExit("phase11 sanitizer matrix fixture version mismatch")
    ...
    for lane_id, blocking in required_map.items():
        ...
        if observed is None:
            lane_results.append({"id": lane_id, "blocking": blocking, "status": "missing", "evidence": ""})
    ...
    blocking_failures = sum(
        1 for lane in lane_results if lane.get("blocking") and lane.get("status") != "pass"
    )
    ...
    (output_dir / "phase11_sanitizer_matrix.md").write_text(make_markdown(payload, output_dir), encoding="utf-8")
    ...
    manifest = {
        ...
        "artifacts": [
            ...
            "phase11_sanitizer_matrix.md",
        ],
    }
    write_json(output_dir / "manifest.json", manifest)

    print(f"phase11-sanitizer-matrix: generated artifacts in {output_dir} (status={status})")
