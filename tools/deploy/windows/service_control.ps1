param(
  [ValidateSet("start", "stop", "status", "enable")]
  [string]$Action = "",
...
for ($index = 0; $index -lt $args.Count; $index++) {
  ...
  switch ($token) {
    "--action" { $index++; if ($index -lt $args.Count) { $Action = [string]$args[$index] }; continue }
    "-Action" { $index++; if ($index -lt $args.Count) { $Action = [string]$args[$index] }; continue }
...
function Resolve-PowerShellPath {
  ...
  Emit-ArlenServiceError "powershell_not_found" "arlen service ${Action}: unable to locate powershell.exe"
}
...
function Invoke-ArlenServiceElevation {
  ...
  try {
    ...
    Emit-ArlenServiceError "uac_cancelled" "arlen service ${Action}: elevation was cancelled or denied."
  }
...
if (-not $Action) {
  Emit-ArlenServiceError "missing_action" "arlen service: --action start|stop|status|enable is required" 2
}
if (-not $Mode) {
  Emit-ArlenServiceError "missing_mode" "arlen service ${Action}: --mode dev|runtime is required" 2
}
...
if ($Mode -eq "dev") {
  if ($AppRoot) {
    if (-not (Test-ArlenAppRoot $AppRoot)) {
      Emit-ArlenServiceError "invalid_app_root" "arlen service ${Action}: --app-root is not a supported Arlen app root: $AppRoot"
    }
    ...
    if (-not $resolvedAppRoot) {
      Emit-ArlenServiceError "app_root_unresolved" "arlen service ${Action}: could not auto-discover app root from $workingDirectory"
    }
  }
} else {
  if ($ReleasesDir) {
    if (-not (Test-ArlenReleasesDir $ReleasesDir)) {
      Emit-ArlenServiceError "invalid_releases_dir" "arlen service ${Action}: --releases-dir does not point to a releases directory: $ReleasesDir"
    }
    ...
    if (-not $resolvedReleasesDir) {
      Emit-ArlenServiceError "releases_dir_unresolved" "arlen service ${Action}: could not auto-discover releases directory from $workingDirectory"
    }
  }
}
...
if (-not $serviceName) {
  Emit-ArlenServiceError "invalid_service_name" "arlen service ${Action}: service name resolved to empty output"
}

$service = Get-Service -Name $serviceName -ErrorAction SilentlyContinue
if (-not $service) {
  Emit-ArlenServiceError "service_not_found" "arlen service ${Action}: service not found: $serviceName"
}

$wmiService = Get-CimInstance -ClassName Win32_Service -Filter "Name='$serviceName'" -ErrorAction SilentlyContinue
$startupType = if ($wmiService) { $wmiService.StartMode } else { "" }
$state = if ($wmiService) { $wmiService.State } else { $service.Status.ToString() }

$plan = @{
  version = "phase7g-agent-dx-contracts-v1"
  command = "service"
  workflow = "service.$Action"
  status = if ($DryRun -or $Action -eq "status") { "planned" } else { "ok" }
  action = $Action
  mode = $Mode
  name = $serviceName
...
if (-not (Test-IsElevated)) {
  Invoke-ArlenServiceElevation -SelectedAction $Action `
                               -SelectedMode $Mode `
...
if (-not (Write-ArlenServicePayload -Payload $plan)) {
  ...
  Write-Host "  startup type: $($plan.startup_type)"
}
