...
- Phase 36: in progress. Delivered so far: `deploy list`, canonical `deploy dryrun` with `plan` alias compatibility, `deploy releases`, and named-target release reuse for pushed remote artifacts. Remaining work covers deploy config samples, initialized-target guards, and bash/PowerShell completions. See `docs/PHASE36_ROADMAP.md`.
