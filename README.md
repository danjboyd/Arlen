...
- Phase 32: complete (`32A-32L` delivered on 2026-04-14). Arlen now ships the full target-aware deployment closeout: platform profiles, support-level classification (`supported` / `experimental` / `unsupported`), declarative deploy-target metadata, remote rebuild gating with explicit build-chain validation, `phase32-deploy-manifest-v1`, explicit `propane_handoff` metadata, deeper deploy `doctor` / `status` / `rollback` metadata, and the `phase32-confidence` artifact lane. See `docs/PHASE32_ROADMAP.md` and `docs/DEPLOYMENT.md`.
