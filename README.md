...
- Phase 32: complete (`32A-32Q` delivered on 2026-04-14). Arlen now ships the full target-aware deployment closeout: platform profiles, support-level classification (`supported` / `experimental` / `unsupported`), declarative deploy-target metadata, remote rebuild gating with explicit build-chain validation, `phase32-deploy-manifest-v1`, explicit `propane_handoff` metadata, explicit database/configuration deploy contracts, doctor validation for declared database mode and required env keys, live runtime-root conflict detection, `deploy release` runtime-action parity, and the `phase32-confidence` artifact lane. See `docs/PHASE32_ROADMAP.md` and `docs/DEPLOYMENT.md`.
