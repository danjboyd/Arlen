...
- `docs/WINDOWS_CLANG64.md` for native Windows CLANG64 support
- `docs/README.md`
...
Arlen now supports native Windows development, runtime, and release workflows
through the checked-in MSYS2 `CLANG64` contract on branch `windows/clang64`.
That supported path covers the CLI, the default `make test-unit` /
`make test-integration` entrypoints, PostgreSQL and MSSQL live-backend suites,
the broader Windows parity workflow, app-root
`boomhauer`/`jobs worker`/`propane` flows, immutable release packaging, and
PowerShell release helpers for migrate/start/reload/stop operations.
...
- Phase 24: in progress on branch `windows/clang64` (`24A-24T` delivered on 2026-04-07 for the checked-in MSYS2 `CLANG64` Windows parity, release contract, and PowerShell-first launcher wrappers; `24U` is planned for Windows-native `arlen service` install/uninstall support with Linux `systemd` wiring to follow later behind the same CLI surface. See `docs/PHASE24_ROADMAP.md`).
