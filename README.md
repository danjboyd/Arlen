...
- Phase 36: in progress. Delivered so far: `36A-36H`, covering `deploy list`, canonical `deploy dryrun` with `plan` alias compatibility, `deploy releases`, named-target release reuse for pushed remote artifacts, initialized-target guards, deploy config samples, `deploy target sample`, and bash/PowerShell completion generation. Remaining work covers completion safety closeout, docs refresh, `phase36-confidence`, and phase closeout. See `docs/PHASE36_ROADMAP.md`.
