...
- Phase 36: planned. This deploy operator-UX phase will add `deploy list`, rename `deploy plan` to `deploy dryrun` with an alias, list available release artifacts, ship deploy config samples, guard uninitialized remote targets, and add bash/PowerShell completions. See `docs/PHASE36_ROADMAP.md`.
