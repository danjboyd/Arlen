...
- Phase 32: in progress (`32A-32I` delivered on 2026-04-14). Arlen now has a target-aware deployment contract plus initial enforcement: platform profiles, support-level classification (`supported` / `experimental` / `unsupported`), declarative deploy-target metadata, remote rebuild gating with explicit build-chain validation, `phase32-deploy-manifest-v1`, and deeper deploy `doctor` / `status` / `rollback` metadata. See `docs/PHASE32_ROADMAP.md` and `docs/DEPLOYMENT.md`.
