...
- `target sample` prints or writes the canonical commented
  `config/deploy.plist.example`; `--write`, `--force`, `--target <name>`,
  `--ssh-host <host>`, and `--output <path>` control generated content and
  destination
- `push` builds a local immutable release artifact under `releases/<release-id>/`
- `push <target>` and `release <target>` fail before remote mutation with
  `deploy_target_not_initialized` when SSH target host artifacts have not been
  generated by `arlen deploy init <target>`
...
### 4.5B `arlen completion <bash|powershell>`

- emits first-class shell completion scripts to stdout
- bash output registers `arlen` through `complete -F _arlen_complete arlen`
- PowerShell output registers `arlen` through `Register-ArgumentCompleter`
- generated completers call `arlen completion candidates ...` for local
  dynamic candidates
- candidate commands cover top-level commands, deploy subcommands, deploy
  target names, local release IDs, and common deploy options
- completion candidates are read-only, local-only, and tolerant of missing or
  half-edited deploy config
