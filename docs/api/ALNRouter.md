...
| `addRouteMethod:path:name:formats:controllerClass:guardAction:action:policies:` | `- (ALNRoute *)addRouteMethod:(NSString *)method path:(NSString *)path name:(nullable NSString *)name formats:(nullable NSArray *)formats controllerClass:(Class)controllerClass guardAction:(nullable NSString *)guardAction action:(NSString *)action policies:(nullable NSArray *)policies;` | Register a route and attach named route policies. | Use policy names that exist under `security.routePolicies`; unknown names fail application startup. |
...
| `matchMethod:path:format:params:` | `- (nullable ALNRoute *)matchMethod:(NSString *)method path:(NSString *)path format:(nullable NSString *)format params:(NSDictionary *_Nullable *_Nullable)params;` | Match input against the configured pattern set. | Capture the returned value and propagate errors/validation as needed. |
