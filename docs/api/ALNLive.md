...
| `keyedTargetSelectorForContainer:key:` | `+ (NSString *)keyedTargetSelectorForContainer:(nullable NSString *)container key:(nullable NSString *)key;` | Perform `keyed target selector for container` for `ALNLive`. | Call on the class type, not on an instance. |
| `upsertKeyedOperationForContainer:key:html:prepend:` | `+ (NSDictionary *)upsertKeyedOperationForContainer:(nullable NSString *)container key:(nullable NSString *)key html:(NSString *)html prepend:(BOOL)prepend;` | Perform `upsert keyed operation for container` for `ALNLive`. | Call on the class type, not on an instance. |
| `removeKeyedOperationForContainer:key:` | `+ (NSDictionary *)removeKeyedOperationForContainer:(nullable NSString *)container key:(nullable NSString *)key;` | Perform `remove keyed operation for container` for `ALNLive`. | Call on the class type, not on an instance. |
