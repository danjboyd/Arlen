...
| `refreshSigningKeysWithError:` | `- (BOOL)refreshSigningKeysWithError:(NSError *_Nullable *_Nullable)error;` | Refresh trusted discovery/JWKS with bounded single-flight fetching and cooldown. | Call before startup or on one dedicated application maintenance worker. Never call from a serialized HTTP loop. NO can mean cooldown or fetch failure; use isReady to check whether the existing snapshot is still fresh. |
| `isReady` | `- (BOOL)isReady;` | Check for a fresh trusted discovery/JWKS snapshot without network I/O. | Integrate into private application readiness. This does not verify every incoming key ID, client sign-in or application permissions. The built-in /readyz is not automatically changed. |
