...
| `renderFileAtPath:contentType:options:` | `- (BOOL)renderFileAtPath:(NSString *)path contentType:(nullable NSString *)contentType options:(nullable NSDictionary *)options;` | Render a response payload for the current request context. | Call from controller action paths after selecting response status/headers. |
| `isLiveRequest` | `- (BOOL)isLiveRequest;` | Return whether `ALNController` currently satisfies this condition. | Check the return value to confirm the operation succeeded. |
| `liveMetadata` | `- (NSDictionary *)liveMetadata;` | Perform `live metadata` for `ALNController`. | Read this value when you need current runtime/request state. |
| `renderLiveOperations:error:` | `- (BOOL)renderLiveOperations:(NSArray *)operations error:(NSError *_Nullable *_Nullable)error;` | Render a response payload for the current request context. | Check the returned `BOOL`; on `NO`, inspect the `error` out-parameter. Call from controller action paths after selecting response status/headers. |
| `renderLiveTemplate:target:action:context:error:` | `- (BOOL)renderLiveTemplate:(NSString *)templateName target:(nullable NSString *)target action:(nullable NSString *)action context:(nullable NSDictionary *)context error:(NSError *_Nullable *_Nullable)error;` | Render a response payload for the current request context. | Check the returned `BOOL`; on `NO`, inspect the `error` out-parameter. Call from controller action paths after selecting response status/headers. |
| `renderLiveKeyedTemplate:container:key:prepend:context:error:` | `- (BOOL)renderLiveKeyedTemplate:(NSString *)templateName container:(nullable NSString *)container key:(nullable NSString *)key prepend:(BOOL)prepend context:(nullable NSDictionary *)context error:(NSError *_Nullable *_Nullable)error;` | Render a response payload for the current request context. | Check the returned `BOOL`; on `NO`, inspect the `error` out-parameter. Call from controller action paths after selecting response status/headers. |
| `renderLiveNavigateTo:replace:` | `- (void)renderLiveNavigateTo:(NSString *)location replace:(BOOL)replace;` | Render a response payload for the current request context. | Call from controller action paths after selecting response status/headers. |
| `publishLiveOperations:onChannel:error:` | `- (NSUInteger)publishLiveOperations:(NSArray *)operations onChannel:(NSString *)channel error:(NSError *_Nullable *_Nullable)error;` | Publish a message to channel subscribers. | Pass `NSError **` when you need detailed failure diagnostics. |
| `publishLiveKeyedTemplate:container:key:prepend:context:onChannel:error:` | `- (NSUInteger)publishLiveKeyedTemplate:(NSString *)templateName container:(nullable NSString *)container key:(nullable NSString *)key prepend:(BOOL)prepend context:(nullable NSDictionary *)context onChannel:(NSString *)channel error:(NSError *_Nullable *_Nullable)error;` | Publish a message to channel subscribers. | Pass `NSError **` when you need detailed failure diagnostics. |
...
| `dataverseClient` | `- (nullable ALNDataverseClient *)dataverseClient;` | Perform `dataverse client` for `ALNController`. | Read this value when you need current runtime/request state. |
| `dataverseClientNamed:error:` | `- (nullable ALNDataverseClient *)dataverseClientNamed:(nullable NSString *)targetName error:(NSError *_Nullable *_Nullable)error;` | Perform `dataverse client named` for `ALNController`. | Pass `NSError **` and treat a `nil` result as failure. |
| `dataverseTargetNames` | `- (NSArray<NSString *> *)dataverseTargetNames;` | Perform `dataverse target names` for `ALNController`. | Read this value when you need current runtime/request state. |
...
| `eventStreamStore` | `- (nullable id<ALNEventStreamStore>)eventStreamStore;` | Perform `event stream store` for `ALNController`. | Read this value when you need current runtime/request state. |
| `eventStreamBroker` | `- (nullable id<ALNEventStreamBroker>)eventStreamBroker;` | Perform `event stream broker` for `ALNController`. | Read this value when you need current runtime/request state. |
| `appendEventStreamEvent:toStream:error:` | `- (nullable ALNEventStreamAppendResult *)appendEventStreamEvent:(NSDictionary *)event toStream:(NSString *)streamID error:(NSError *_Nullable *_Nullable)error;` | Perform `append event stream event` for `ALNController`. | Pass `NSError **` and treat a `nil` result as failure. |
| `replayEventStream:afterSequence:limit:replayWindow:error:` | `- (nullable ALNEventStreamReplayResult *)replayEventStream:(NSString *)streamID afterSequence:(nullable NSNumber *)sequence limit:(NSUInteger)limit replayWindow:(NSUInteger)replayWindow error:(NSError *_Nullable *_Nullable)error;` | Perform `replay event stream` for `ALNController`. | Pass `NSError **` and treat a `nil` result as failure. |
| `renderEventStreamReplay:afterSequence:limit:replayWindow:error:` | `- (BOOL)renderEventStreamReplay:(NSString *)streamID afterSequence:(nullable NSNumber *)sequence limit:(NSUInteger)limit replayWindow:(NSUInteger)replayWindow error:(NSError *_Nullable *_Nullable)error;` | Render a response payload for the current request context. | Check the returned `BOOL`; on `NO`, inspect the `error` out-parameter. Call from controller action paths after selecting response status/headers. |
| `renderSSEStream:afterSequence:limit:replayWindow:error:` | `- (BOOL)renderSSEStream:(NSString *)streamID afterSequence:(nullable NSNumber *)sequence limit:(NSUInteger)limit replayWindow:(NSUInteger)replayWindow error:(NSError *_Nullable *_Nullable)error;` | Render a response payload for the current request context. | Check the returned `BOOL`; on `NO`, inspect the `error` out-parameter. Call from controller action paths after selecting response status/headers. |
| `acceptWebSocketStream:afterSequence:limit:replayWindow:error:` | `- (BOOL)acceptWebSocketStream:(NSString *)streamID afterSequence:(nullable NSNumber *)sequence limit:(NSUInteger)limit replayWindow:(NSUInteger)replayWindow error:(NSError *_Nullable *_Nullable)error;` | Accept and handle an upgraded realtime connection. | Check the returned `BOOL`; on `NO`, inspect the `error` out-parameter. |
