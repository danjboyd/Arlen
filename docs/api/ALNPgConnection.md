...
| `createSavepointNamed:error:` | `- (BOOL)createSavepointNamed:(NSString *)name error:(NSError *_Nullable *_Nullable)error;` | Perform `create savepoint named` for `ALNPgConnection`. | Check the returned `BOOL`; on `NO`, inspect the `error` out-parameter. |
| `rollbackToSavepointNamed:error:` | `- (BOOL)rollbackToSavepointNamed:(NSString *)name error:(NSError *_Nullable *_Nullable)error;` | Perform `rollback to savepoint named` for `ALNPgConnection`. | Check the returned `BOOL`; on `NO`, inspect the `error` out-parameter. |
| `releaseSavepointNamed:error:` | `- (BOOL)releaseSavepointNamed:(NSString *)name error:(NSError *_Nullable *_Nullable)error;` | Perform `release savepoint named` for `ALNPgConnection`. | Check the returned `BOOL`; on `NO`, inspect the `error` out-parameter. |
