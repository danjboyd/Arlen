...
| `eventStreamStore` | `- (nullable id<ALNEventStreamStore>)eventStreamStore;` | Perform `event stream store` for `ALNContext`. | Read this value when you need current runtime/request state. |
| `eventStreamBroker` | `- (nullable id<ALNEventStreamBroker>)eventStreamBroker;` | Perform `event stream broker` for `ALNContext`. | Read this value when you need current runtime/request state. |
