...
| `policyNames` | `NSArray *` | `nonatomic, copy` | Public `policyNames` property available on `ALNRoute`. |
| `source` | `NSString *` | `nonatomic, copy` | Public `source` property available on `ALNRoute`. |
