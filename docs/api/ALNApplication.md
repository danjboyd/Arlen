...
| `eventStreamStore` | `id<ALNEventStreamStore>` | `nonatomic, strong, readonly, nullable` | Public `eventStreamStore` property available on `ALNApplication`. |
| `eventStreamBroker` | `id<ALNEventStreamBroker>` | `nonatomic, strong, readonly, nullable` | Public `eventStreamBroker` property available on `ALNApplication`. |
| `eventStreamAuthorizationHook` | `id<ALNEventStreamAuthorizationHook>` | `nonatomic, strong, readonly, nullable` | Public `eventStreamAuthorizationHook` property available on `ALNApplication`. |
...
| `registerRouteMethod:path:name:formats:controllerClass:guardAction:action:policies:` | `- (ALNRoute *)registerRouteMethod:(NSString *)method path:(NSString *)path name:(nullable NSString *)name formats:(nullable NSArray *)formats controllerClass:(Class)controllerClass guardAction:(nullable NSString *)guardAction action:(NSString *)actionName policies:(nullable NSArray *)policies;` | Register this component so it participates in runtime behavior. | Call during bootstrap/setup before this behavior is exercised. |
...
| `appRootPath` | `- (NSString *)appRootPath;` | Perform `app root path` for `ALNApplication`. | Read this value when you need current runtime/request state. |
| `pathRelativeToAppRoot:` | `- (NSString *)pathRelativeToAppRoot:(NSString *)path;` | Perform `path relative to app root` for `ALNApplication`. | Capture the returned value and propagate errors/validation as needed. |
...
| `setEventStreamStore:` | `- (void)setEventStreamStore:(id<ALNEventStreamStore>)adapter;` | Set or override the current value for this concern. | Call before downstream behavior that depends on this updated value. |
| `setEventStreamBroker:` | `- (void)setEventStreamBroker:(id<ALNEventStreamBroker>)adapter;` | Set or override the current value for this concern. | Call before downstream behavior that depends on this updated value. |
| `setEventStreamAuthorizationHook:` | `- (void)setEventStreamAuthorizationHook:(id<ALNEventStreamAuthorizationHook>)hook;` | Set or override the current value for this concern. | Call before downstream behavior that depends on this updated value. |
| `setDataverseClient:forTargetName:` | `- (void)setDataverseClient:(ALNDataverseClient *)client forTargetName:(nullable NSString *)targetName;` | Set or override the current value for this concern. | Call before downstream behavior that depends on this updated value. |
| `dataverseClient` | `- (nullable ALNDataverseClient *)dataverseClient;` | Perform `dataverse client` for `ALNApplication`. | Read this value when you need current runtime/request state. |
| `dataverseClientNamed:error:` | `- (nullable ALNDataverseClient *)dataverseClientNamed:(nullable NSString *)targetName error:(NSError *_Nullable *_Nullable)error;` | Perform `dataverse client named` for `ALNApplication`. | Pass `NSError **` and treat a `nil` result as failure. |
| `dataverseTargetNamed:error:` | `- (nullable ALNDataverseTarget *)dataverseTargetNamed:(nullable NSString *)targetName error:(NSError *_Nullable *_Nullable)error;` | Perform `dataverse target named` for `ALNApplication`. | Pass `NSError **` and treat a `nil` result as failure. |
| `dataverseTargetNames` | `- (NSArray<NSString *> *)dataverseTargetNames;` | Perform `dataverse target names` for `ALNApplication`. | Read this value when you need current runtime/request state. |
...
| `configureAuthAssuranceForRouteNamed:minimumAuthAssuranceLevel:maximumAuthenticationAgeSeconds:stepUpPath:error:` | `- (BOOL)configureAuthAssuranceForRouteNamed:(NSString *)routeName minimumAuthAssuranceLevel:(NSUInteger)minimumAuthAssuranceLevel maximumAuthenticationAgeSeconds:(NSUInteger)maximumAuthenticationAgeSeconds stepUpPath:(nullable NSString *)stepUpPath error:(NSError *_Nullable *_Nullable)error;` | Configure behavior for an already-registered runtime element. | Check the returned `BOOL`; on `NO`, inspect the `error` out-parameter. |
| `configureRoutePoliciesForRouteNamed:policies:error:` | `- (BOOL)configureRoutePoliciesForRouteNamed:(NSString *)routeName policies:(nullable NSArray *)policies error:(NSError *_Nullable *_Nullable)error;` | Configure behavior for an already-registered runtime element. | Check the returned `BOOL`; on `NO`, inspect the `error` out-parameter. |
| `authorizeEventStreamAppendToStream:event:context:error:` | `- (BOOL)authorizeEventStreamAppendToStream:(NSString *)streamID event:(NSDictionary *)event context:(nullable ALNContext *)context error:(NSError *_Nullable *_Nullable)error;` | Perform `authorize event stream append to stream` for `ALNApplication`. | Check the returned `BOOL`; on `NO`, inspect the `error` out-parameter. |
| `authorizeEventStreamReplayOfStream:afterSequence:context:error:` | `- (BOOL)authorizeEventStreamReplayOfStream:(NSString *)streamID afterSequence:(nullable NSNumber *)sequence context:(nullable ALNContext *)context error:(NSError *_Nullable *_Nullable)error;` | Perform `authorize event stream replay of stream` for `ALNApplication`. | Check the returned `BOOL`; on `NO`, inspect the `error` out-parameter. |
| `authorizeEventStreamSubscribeToStream:context:error:` | `- (BOOL)authorizeEventStreamSubscribeToStream:(NSString *)streamID context:(nullable ALNContext *)context error:(NSError *_Nullable *_Nullable)error;` | Perform `authorize event stream subscribe to stream` for `ALNApplication`. | Check the returned `BOOL`; on `NO`, inspect the `error` out-parameter. |
...
| `dispatchRequest:requiringRoute:` | `- (ALNResponse *)dispatchRequest:(ALNRequest *)request requiringRoute:(nullable ALNRoute *)route;` | Dispatch the current request through routing and controller handling. | Capture the returned value and propagate errors/validation as needed. |
