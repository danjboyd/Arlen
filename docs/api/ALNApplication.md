...
| `registerRouteMethod:path:name:formats:controllerClass:guardAction:action:policies:` | `- (ALNRoute *)registerRouteMethod:(NSString *)method path:(NSString *)path name:(nullable NSString *)name formats:(nullable NSArray *)formats controllerClass:(Class)controllerClass guardAction:(nullable NSString *)guardAction action:(NSString *)actionName policies:(nullable NSArray *)policies;` | Register a route and attach named route policies. | Use policy names that exist under `security.routePolicies`; unknown names fail application startup. |
...
| `configureRoutePoliciesForRouteNamed:policies:error:` | `- (BOOL)configureRoutePoliciesForRouteNamed:(NSString *)routeName policies:(nullable NSArray *)policies error:(NSError *_Nullable *_Nullable)error;` | Attach named route policies to an already-registered route. | Call after route registration and before startup; unknown names fail application startup. |
