...
- [TaxCalculator Health Startup Reconciliation](TAXCALCULATOR_HEALTH_STARTUP_RECONCILIATION_2026-05-02.md): upstream status for the May 2, 2026 post-restart health probe race report.
...
- [Phase 39 Roadmap](PHASE39_ROADMAP.md): planned multi-worker state-safety phase covering process-local state documentation, durable state contracts, doctor/deploy warnings, demo-only in-memory labeling, durable store examples, optional worker identity diagnostics, and confidence coverage.
...
- [Phase 37 Roadmap](PHASE37_ROADMAP.md): completed public-release test confidence phase; `37A-37S` delivered the public-surface contract, EOC golden-render catalog, deterministic parser/protocol corpus, acceptance harness, EOC/MVC/module/data/live/deploy acceptance sites, runtime-mode slots, executable golden assertions, regression intake enforcement, packaged deploy proof, coverage status tracking, `phase37-confidence`, and closeout docs.
...
- [TaxCalculator Report Reconciliation (2026-04-30)](TAXCALCULATOR_REPORT_RECONCILIATION_2026-04-30.md): upstream closure note for remote deploy `sshOptions` ordering, early SSH-exit upload failures, and shell-command capture deadlocks reported from `TaxCalculator`.
