...
- [Phase 36 Roadmap](PHASE36_ROADMAP.md): deploy operator-UX phase; 36A-36D are delivered for `deploy list`, `deploy dryrun`, release inventories, and named-target release reuse, with deploy config samples, initialized-target guards, and bash/PowerShell completions still planned.
