...
- [Phase 36 Roadmap](PHASE36_ROADMAP.md): deploy operator-UX phase; 36A-36H are delivered for `deploy list`, `deploy dryrun`, release inventories, named-target release reuse, initialized-target guards, deploy config samples, `deploy target sample`, and bash/PowerShell completion generation, with completion safety closeout, docs refresh, confidence, and phase closeout still planned.
