...
| Descriptor-first TypeScript client/model generation for React and SPA consumers | Prisma, MikroORM, Drizzle ecosystems | In Scope | Phase 28 (complete) | Generate TypeScript contracts, clients, validators, query/resource metadata, and optional React helpers from Arlen descriptors plus route/OpenAPI metadata without making TypeScript the canonical ORM source. |
...
| Server-driven live fragments, forms, keyed collections, and navigation baseline | Phoenix LiveView, Hotwire/Turbo, Symfony UX LiveComponent, Livewire | In Scope | Phase 25 complete | Fragment-first HTML-over-the-wire baseline ships with `ALNLive`, `/arlen/live.js`, live request metadata, keyed collection helpers, live regions, upload-progress-aware forms, websocket-backed push updates, executable runtime semantics coverage in `phase25-live-tests`, and adversarial/confidence closeout through `25L`. |
...
| Distributed runtime failure-depth contracts (quorum + coordination matrix) | Phoenix | In Scope | Phase 7H (initial slice complete) | Added quorum-gated readiness semantics, expanded `/clusterz` coordination boundaries, and deterministic cluster diagnostics headers for incident triage. |
