...
| Server-driven live fragments, forms, keyed collections, and navigation baseline | Phoenix LiveView, Hotwire/Turbo, Symfony UX LiveComponent, Livewire | In Scope | Phase 25 (baseline + `25H-25J` hardening delivered) | Fragment-first HTML-over-the-wire baseline ships with `ALNLive`, `/arlen/live.js`, live request metadata, keyed collection helpers, live regions, upload-progress-aware forms, websocket-backed push updates, and executable runtime semantics coverage in `phase25-live-tests`. |
