...
- (void)testArlenSchemaCodegenTypedContractsCompileAndDecodeDeterministically {
  ...
  XCTAssertEqualObjects(@1, manifest[@"reflection_contract_version"]);
  ...
  NSArray *columnMetadata =
      [generatedTable[@"column_metadata"] isKindOfClass:[NSArray class]] ? generatedTable[@"column_metadata"] : nil;
...
- (void)testDatabaseRouterReadWriteRoutingAcrossLiveAdapters {
  ...
  NSString *source = [NSString stringWithFormat:
      @"#import <Foundation/Foundation.h>\n"
       ...
       "                                                    parameters:@[]\n"
       "                                                         error:&error];\n"
       "    NSNumber *count = [[rows firstObject][@\"count\"] isKindOfClass:[NSNumber class]] ? [rows firstObject][@\"count\"] : nil;\n"
       "    if (![count isEqual:@1] || error != nil) {\n"
       ...
       "                routingContext:scope\n"
       "                         error:&error];\n"
       "    count = [[rows firstObject][@\"count\"] isKindOfClass:[NSNumber class]] ? [rows firstObject][@\"count\"] : nil;\n"
       "    if (![count isEqual:@2] || error != nil) {\n"
