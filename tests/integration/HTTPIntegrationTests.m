...
- (void)testProductionModeDispatchesConcurrentlyByDefault {
  int port = [self randomPort];
  NSTask *server = [[NSTask alloc] init];
  server.launchPath = @"./build/boomhauer";
...
- (void)testProductionRequestDispatchModeCanBeOverriddenToSerialized {
  ...
  server.arguments = @[
    @"-lc",
    [NSString stringWithFormat:
                  @"ARLEN_REQUEST_DISPATCH_MODE=serialized ./build/boomhauer --env production --port %d",
                  port]
  ];
  server.standardOutput = [NSPipe pipe];
  server.standardError = [NSPipe pipe];
...
- (void)testProductionModeMixedLifecycleStressRemainsStable {
...
- (void)testBoomhauerWatchServesBuildErrorPageAndRecoversAfterFix {
  ...
  NSString *appSourceTemplate = @"#import <Foundation/Foundation.h>\n"
                                ...
                                "  }\n"
                                "}\n";
  NSString *brokenAppSource =
      [appSourceTemplate stringByReplacingOccurrencesOfString:@"%BUILD_SENTINEL%"
                                                    withString:@"#error WATCH_BUILD_TOGGLE "];
  NSString *fixedAppSource =
      [appSourceTemplate stringByReplacingOccurrencesOfString:@"%BUILD_SENTINEL%"
                                                    withString:@"#define WATCH_BUILD_TOGGLE"];
