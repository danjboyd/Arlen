...
This example is the live backend used by the TypeScript and React
integration lanes.
...
ARLEN_APP_ROOT=examples/typescript_reference_server ./build/phase28-reference-server --port 3140
```

The repo-native CI scripts fetch `/openapi.json`, merge in the
checked-in `x-arlen` metadata, assert that the live exported contract
